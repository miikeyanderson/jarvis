 ...
 *      environment variable JARVIS_RUST to a truthy value (`1`, `true`, etc.).
 ...
 *
 *      If the JARVIS_RUST=1 is specified and there is no native binary for the
...
// setting JARVIS_RUST=1 will opt-in to the native binary, if included.
const wantsNative = fs.existsSync(path.join(__dirname, "use-native")) ||
  (process.env.JARVIS_RUST != null
    ? ["1", "true", "yes"].includes(process.env.JARVIS_RUST.toLowerCase())
    : false);
...
if (wantsNative) {
  ...
  const binaryPath = path.join(__dirname, "..", "bin", `jarvis-${targetTriple}`);
